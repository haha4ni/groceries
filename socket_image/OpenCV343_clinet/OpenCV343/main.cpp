...
{
	/*
	cv::Mat img = cv::imread("all.jpg", CV_LOAD_IMAGE_COLOR);
	cv::imshow("aaa", img);
	cv::waitKey(0);
	*/
	/*
	...
	*/
	...
	//sConnection = socket(PF_INET, SOCK_STREAM, NULL);

	// 設定位址資訊的資料
	//addr.sin_addr.s_addr = inet_addr("140.118.175.168");
	addr.sin_addr.s_addr = INADDR_ANY;//自動填入IP
	...
	while (1)
	{
		std::cout << "Waitting for connect... " << std::endl;
		if (sConnection = accept(sListen, (SOCKADDR*)&clientAddr, &addrlen))
		{
			...
			int bufferLength;
			int r1 = recv(sConnection, (char*)&bufferLength, 4, 0);


			uchar* buff = new uchar[bufferLength];
			ZeroMemory(buff, bufferLength);
			int r2 = recv(sConnection, (char*)buff, bufferLength, 0);
			...
			cv::Mat img_decode = cv::imdecode(data, CV_LOAD_IMAGE_UNCHANGED);
